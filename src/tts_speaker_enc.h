 ...
 * and computes speaker embeddings from 128-band mel spectrograms.
 *
 * Architecture:
 ...
 *   FC:        Conv1d(3072->embed_dim, k=1) -> speaker embedding
 *
 * embed_dim: 1024 (0.6B model) or 2048 (1.7B model), detected at init.
 */
...
typedef struct {
    ...
    /* FC: Conv1d(3072->embed_dim, k=1) */
    float *fc_weight;       /* [embed_dim, 3072] */
    float *fc_bias;         /* [embed_dim] */

    int embed_dim;          /* 1024 (0.6B) or 2048 (1.7B), detected from fc.weight */
...
/* Forward pass: mel [128, n_frames] -> out_embedding [embed_dim].
